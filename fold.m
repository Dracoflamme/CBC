...
%   Detailed explanation goes here
    ...
    for i = 1:10
       ...
       if (i==10) finish = num_exs; %ensure we test on all data
        end
       data.test(i).labels = labels(start:finish-validation_size);
       data.test(i).examples = examples(start:finish-validation_size,:);
       data.train(i).labels = cat(1,labels(1:start - 1),labels(finish+1:end));
       data.train(i).examples = cat(1,examples(1:start- 1,:),examples(finish+1:end,:));
