function [ rates ] = crossValidateANN( labels, examples, one_or_six,optimise_parameters,params)
    ...
    if strcmp(optimise_parameters,'yes')
        [topology,train_fn,learn_rate,momentum,activation_fn,epochs] = paramOpt(fold(1).train,fold(1).validate);
    else
        topology = params(1);
        train_fn = params(2);
        learn_rate = params(3);
    ...
    for i = 1:length(folds)
        ...
        predictions = testANN(network,fold(i).examples);
        rates(i) = classificationRate(predictions,fold(i).test.labels);
    end
end
